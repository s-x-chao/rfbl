...
public class FantasyPro_SP_Ranks_CSV extends Parser{

	private static final Logger log = LoggerFactory.getLogger(PlayerSearcher.class);

	
	public FantasyPro_SP_Ranks_CSV() {
	}

	public FantasyPro_SP_Ranks_CSV(String file){
		super(file, "202404");
	}
	...
	public void parsePlayers() {
		...
		try (Stream<String> stream = Files.lines(Paths.get(file.getAbsolutePath()))) {
			...
			for (String playerLine : lines){
				...
				player.setEligible_positions(new String[] {"SP"});
				try {
